 ...
 *
 * REC-153 · Esto vivía en `/`. Se mudó acá para liberar la raíz, que pasa a ser
 * la landing pública (REC-154). La lógica NO cambió: el `switch`, el `never` y
 * las ramas son las mismas que estaban en `src/app/page.tsx`. Durante la
 * transición `/` es un `redirect` a esta ruta, así que ninguna pestaña vieja
 * queda huérfana; el `/` de verdad se reemplaza recién con la landing.
 *
 ...
 * `OnboardingView` ve `rol !== "damnificado"` y hace `router.replace(RUTAS.inicio)`
